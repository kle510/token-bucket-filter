...
void *PacketArrival(void *arg)
{
	...
	for (;;)
	{
		if (curr_packet_num <= num)
		{
			...
			double real_interarrival_time = end_interarrival_time - start_interarrival_time;
			total_interarrival_time += real_interarrival_time;

			packets_arrived++;

			if (data->token_req > token_bucket_capacity)
			{ 
				// Drop the packet
				packets_dropped++;
				printf(
			...
			{ 
				// The packet arrives, add it to Q1
		...
		if (curr_packet_num > num && LinkedListEmpty(&Q1))
		{
			...
			packet_shutdown = 1;
			if (packet_shutdown == 1 && token_shutdown == 1 && LinkedListEmpty(&Q1) && LinkedListEmpty(&Q2))
			{
				cancel = 1;
			}
...
void *TokenArrival(void *arg)
{
	...
	for (;;)
	{
		...
		if (token_bucket >= token_bucket_capacity)
		{ 
			// If at capacity, drop the token
...
void *ServerOne(void *arg)
{
	...
	for (;;)
	{
		...
		while (LinkedListEmpty(&Q2) && s2_shutdown == 0 && cancel == 0)
		{ 
			//printf("S1 wait a\n");
		...
		if (s2_shutdown == 1 || cancel == 1)
		{
			...
			if (s1_shutdown == 1 && s2_shutdown == 1 && token_shutdown == 1 && packet_shutdown == 1)
			{
		...
		if (LinkedListEmpty(&Q2) && LinkedListEmpty(&Q1) && last_packet_sent == 1)
		{
			pthread_mutex_lock(&mutex);
			pthread_cond_broadcast(&cv);
			pthread_mutex_unlock(&mutex);
...
void *ServerTwo(void *arg)
{
	...
	for (;;)
	{
		...
		while (LinkedListEmpty(&Q2) && s1_shutdown == 0 && cancel == 0)
		{ // && !shutdown
			//printf("S2 wait a\n");
			pthread_cond_wait(&cv, &mutex);
			//printf("S2 wait b\n");
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i = i + 2)
	{
		if (argc > 1)
		{ 
			if (strcmp(argv[i], "-lambda") != 0 && strcmp(argv[i], "-mu") != 0 && strcmp(argv[i], "-r") != 0 && strcmp(argv[i], "-B") != 0 && strcmp(argv[i], "-P") != 0 && strcmp(argv[i], "-n") != 0 && strcmp(argv[i], "-t") != 0)
			{
				fprintf(stderr,
						"ERROR: malinformed command.  Flag is wrong.\n");
				exit(1);
			}
			if (strcmp(argv[i], "-lambda") == 0 || strcmp(argv[i], "-mu") == 0 || strcmp(argv[i], "-r") == 0 || strcmp(argv[i], "-B") == 0 || strcmp(argv[i], "-P") == 0 || strcmp(argv[i], "-n") == 0)
			{
				char *end;
																			...
																			LinkedListNext(&completed_packet_queue, elem))
	{
		...
		summation = summation + pow((data->time_spent_system - avg_time_spent_system_msec),
									2);
	}
